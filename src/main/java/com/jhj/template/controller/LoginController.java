...
import com.jhj.template.service.UserService;
...
public class LoginController {
	...
	public String getAuthorization(@RequestHeader(value = "Authorization", required = false) String authorization) {
	...
	@Operation(summary = "login", description = "LOGIN TEST", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "로그인 요청", content = @Content(examples = {
			@ExampleObject(name = "id password", value = """
							{
								"username" : "admin",
								"password" : "admin"
							}
	...
	@PostMapping("/logout")
	public Response<String> logout(@RequestHeader(value = "Authorization", required = false) String authorization) {
		...
		try {
			...
			String username = jwtService.extractUsername(accessToken)
					.orElseThrow(() -> new NoSuchElementException("username is not present in accessToken"));
